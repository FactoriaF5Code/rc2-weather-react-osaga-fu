...
const API_CITY = `q={city name}&appid={API key}&cnt=5&lang=es&units=metric`;
...
export default function ForecastHourly({ latitude, longitude }) {
  ...
  return (
    <section className="hourly-forecast">
      ...
      <div className="hourly-cards-container">
        {forecastHourly.list && (
            forecastHourly.list.map((hora)=>( <article className="hourly-forecast-card" key={hora.dt}>
            <h4 className="forecast-card-time">
              {new Date(hora.dt * 1000).getHours()-1}h
            </h4>
            <img className="forecast-weather-icon" src={iconPaths[hora.weather[0].icon]} alt="no hay icono" />
            <h3 className="forecast-card-temp">{hora.main.temp.toFixed(1)}°</h3>
          </article>
         )  )
